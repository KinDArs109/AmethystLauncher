...
:: 65001 - UTF-8
::
:: Self-contained zapret strategy launcher for MC Launcher.
:: Derived from Flowseal/zapret-discord-youtube general.bat (default strategy), but with the
:: interactive service.bat calls (update checks, game-filter/user-list menus) stripped out so it
:: can be started non-interactively by the launcher. Game filter is disabled here (port 12 is the
:: harmless placeholder zapret uses when disabled). Requires admin rights (WinDivert driver).
...
:: Enable TCP timestamps (some strategies rely on them); harmless if already on.
netsh interface tcp set global timestamps=enabled > nul 2>&1

cd /d "%BIN%"

start "zapret: mc-launcher" /min "%BIN%winws.exe" --wf-tcp=80,443,2053,2083,2087,2096,8443,%GameFilterTCP% --wf-udp=443,19294-19344,50000-50100,%GameFilterUDP% ^
